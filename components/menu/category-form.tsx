...
export function CategoryForm({ restaurantId, defaultLanguage, category, onDone }: CategoryFormProps) {
  ...
  const handleTranslate = async () => {
    if (!name.trim()) {
      ...
      toast.error("Por favor ingresa un nombre primero")
      return
    }
    ...
    try {
      ...
      const errorMsg = "Error al traducir. Por favor intenta de nuevo."
      setError(errorMsg)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log("[v0] Submitting category:", { finalTranslations, sortOrder, restaurantId })
